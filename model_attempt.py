...
weather_df_clean = weather_df.drop(columns=["State", "Country",
                         ...
                         "Heat Index", "Wind Direction", "Snow", "Visibility", "Cloud Cover", "Precipitation" ])
...
combined_df = pd.read_sql_query("SELECT weather.city, weather.date_time, weather.Temperature, weather.wind_speed, weather.relative_humidity, weather.Conditions,pollution.city, pollution.date_time, pollution.AQI FROM weather INNER JOIN POLLUTION ON weather.date_time = pollution.date_time WHERE weather.city = pollution.city", con)
